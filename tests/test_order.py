...
def patch_jwt_identity(user_id, role="customer"):
    return patch(
        "route.order_route.get_jwt_identity", return_value={"id": user_id, "role": role}
    )
...
@patch_jwt_identity(1, role="admin")
def test_update_order_status_success(
    mock_get_jwt_identity, client, admin_token, seed_product
):
...
@patch_jwt_identity(1, role="admin")
def test_update_order_status_missing_status(mock_get_jwt_identity, client, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    response = client.put(
        "/orders/1/status",
        data=json.dumps({}),
...
@patch_jwt_identity(1, role="admin")
def test_update_order_status_not_found(mock_get_jwt_identity, client, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    update_data = {"status": "shipped"}
    response = client.put(
